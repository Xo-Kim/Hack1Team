...
PRD([PRD_MCM_Mood_Mirror.md](PRD_MCM_Mood_Mirror.md)) v0.4의 핵심 경로를 구현한 프로토타입.

**촬영(3초 카운트다운) → AI 분석 → 조명 연출 + 음향 재생 + 제품 추천**을 한 화면에서 확인한다.
...
> 현재는 외부 API를 호출한다. 실서비스의 온프렘·폐쇄망 전환은 PRD §16.2 로드맵 참고.
...
| F-9~F-10 직원 알림 · WebSocket | **없음.** STAFF VIEW가 같은 창에서 직접 호출 |
| F-12~F-13 포토 4컷 · QR 전달 | **없음** |
| F-11 응대 모드 | 있음 (조명 유지 + 음량 자동 감소) |
| F-13 리셋 (타임아웃) | 있음 (무입력 3분) |
| F-18 진행 단계 리빌 | 있음 |
