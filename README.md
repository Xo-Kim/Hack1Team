...
**이 라벨은 표시용 문자열이 아니라 직원에게 거기로 걸어가라는 지시다.** 그래서
시스템이 방 이름을 지어내지 않는다 — 없는 방으로 직원을 보내는 것은 두 거울이
구분되지 않는 것보다 나쁘다.

기기마다 URL 을 한 번 지정한다. 키오스크를 설치하는 보통의 방식이고 기기별로
다시 빌드할 필요가 없다.

```
/?mirror=2F-A&label=2F 피팅룸 A     지정된 거울
/?mirror=3F-B                      label 을 생략하면 mirror 값이 그대로 표시된다
/                                  미설정 — 탭마다 "임시 · a3f9"
```

미설정일 때는 탭마다 다른 임시 식별자가 붙는다(`sessionStorage` 라 창을 두 개 열면
서로 다르고, 새로고침해도 그 탭의 값은 유지된다). 라벨은 **일부러 방 이름처럼 보이지
않게** 만들었다. 못생긴 것이 이 상황에서 전달해야 하는 정보다 — 이 거울은 아직
자리를 배정받지 않았다.

`VITE_MIRROR_ID` / `VITE_MIRROR_LABEL` 로 빌드 시점에 박을 수도 있지만, 그러면
기기 수만큼 빌드가 필요하다. URL 쪽을 권한다.
